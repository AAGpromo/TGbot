...
class DBCommands:
    ...
    async def get_users(self):
        command = "SELECT chat_id FROM users"
...
@dp.message_handler(Command("mail"), user_id=admin_id)
async def mailing_start(m: types.Message):
    await m.answer("Отправь текст")
    await Mailing.EnterText.set()


@dp.message_handler(state=Mailing.EnterText)
async def enter_text(m: types.Message, state: FSMContext):
    await state.finish()
    ...
    for (user,) in users:
        try:
            await m.send_copy(chat_id=user)
            await sleep(0.3)
